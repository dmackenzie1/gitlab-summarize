...
def main():
    ...
    if not activity_path.exists():
        ...
        sys.exit(1)
    activity_summary = parse_activity_page(activity_path.read_text())

    logging.info("Step 3/8: Fetching additional data from GitLab API")
    if gitlab_client.token:
        project_stats = gitlab_client.get_group_project_stats(args.group, args.days)
    else:
    ...
    for project_name, stats in project_stats.items():
        ...
        for mr in stats['open_mrs']:
            diff_path = temp_dir / f"{project_name}_{mr['iid']}_diff.txt"
    ...
    report_content = generate_markdown_report(report_path, args.days, activity_summary, project_stats, project_summaries)
